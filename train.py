...
def train(lr,batch_size,max_step,n_classes):
    train_log_dir='./logs_3'
    data_dir='./data/cifar-10-batches-py'
    with tf.name_scope('input'):
        train_img_batch,train_label_batch=input_data_2.read_cifar10(data_dir,is_train=True,batch_size=batch_size,shuffle=True)
        val_img_batch, val_label_batch = input_data_2.read_cifar10(data_dir, is_train=False, batch_size=batch_size,
                                                                     shuffle=False)
    features = tf.placeholder(tf.float32, [batch_size, 32, 32, 3])
    labels = tf.placeholder(tf.int64, [batch_size, 10])
    logits,is_train = resnet_50_2.resnet50(features,n_classes)
...
def evaluate(batch_size):
    with tf.Graph().as_default():
        log_dir='./logs_3'
        test_img_dir='./data/cifar-10-batches-py'
        n_test=20000
        images,labels=input_data_2.read_cifar10(data_dir=test_img_dir,is_train=False,batch_size=batch_size,shuffle=False)
        logits,is_trian=resnet_50_2.resnet50(images,n_claases=10)
